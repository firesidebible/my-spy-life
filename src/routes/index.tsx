...
function Briefing({ onNext }: { onNext: () => void }) {
  ...
  const r = hk.data;
  const bStamina = pct(r?.stand_hours, 12);
  const bStrength = pct(r?.workout_minutes, 60);
  const bIntel = pct(r?.mindful_minutes, 20);
  useEffect(() => {
      ...
      <aside className="flex flex-col gap-4">
        ...
        <Panel title="ASSET · AGENT 86">
          <div className="grid grid-cols-3 gap-2 text-center text-[10px] uppercase tracking-widest">
            <AssetStat label="Stamina" value={bStamina} />
            <AssetStat label="Strength" value={bStrength} />
            <AssetStat label="Intel" value={bIntel} />
          </div>
...
function MissionComplete({ onRestart }: { onRestart: () => void }) {
  const hk = useHealthKitReading();
  const r = hk.data;
  const cStamina = pct(r?.stand_hours, 12);
  const cStrength = pct(r?.workout_minutes, 60);
  const cIntel = pct(r?.mindful_minutes, 20);
  return (
    <div className="mx-auto max-w-3xl">
      <div className="bracket-4 relative border border-hud/50 bg-hud-panel p-10 text-center">
        ...
        <div className="mx-auto mt-8 grid max-w-md grid-cols-3 gap-3 text-[10px] uppercase tracking-widest">
          <Award label="Stamina" v={`${cStamina}%`} />
          <Award label="Strength" v={`${cStrength}%`} />
          <Award label="Intel" v={`${cIntel}%`} />
        </div>
